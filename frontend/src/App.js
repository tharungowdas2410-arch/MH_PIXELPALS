...
const App = () => {
  ...
  const [selectedModels, setSelectedModels] = useState(() => {
    const saved = localStorage.getItem('veristreamSelectedModels');
    return saved ? JSON.parse(saved) : [];
  });
  
  const [threshold, setThreshold] = useState(() => {
    const saved = localStorage.getItem('veristreamThreshold');
    return saved ? parseFloat(saved) : 0.5;
  });
  
  const [ensembleMethod, setEnsembleMethod] = useState(() => {
    const saved = localStorage.getItem('veristreamEnsembleMethod');
    return saved || 'stacking'; 
  });

  const [darkMode, setDarkMode] = useState(() => {
    const saved = localStorage.getItem('veristreamDarkMode');
  ...
  const baseModelInfo = useMemo(() => ({
    "npr_veristream": { name: 'NPR VeriStream', description: 'Neural Pattern Recognition for image deepfakes.', type: 'image' },
  ...
  useEffect(() => {
    localStorage.setItem('veristreamSelectedModels', JSON.stringify(selectedModels));
  }, [selectedModels]);

  useEffect(() => {
    localStorage.setItem('veristreamThreshold', threshold.toString());
  }, [threshold]);

  useEffect(() => {
    localStorage.setItem('veristreamEnsembleMethod', ensembleMethod);
  }, [ensembleMethod]);

  useEffect(() => {
    localStorage.setItem('veristreamDarkMode', darkMode.toString());
